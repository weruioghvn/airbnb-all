...
def iterateDailyDetail(mainResults):
    finalResults = []
    counter = 0
    baseURL = 'https://www.airbnb.com/rooms/'
    ...
    for listing in mainResults:
        counter += 1
        print 'Processing Listing %s out of %s' % (str(counter), str(len(mainResults)))
        priceDict, availabilityDict = getDailyInformation(listing['ListingID'])
        for key in priceDict.keys():
            result = {'listingID': listing['ListingID'],
                      'date': key,
                      'price': priceDict[key],
                      'availability': availabilityDict[key]}
...
def getAboutListing(tree, ListingID):
    ...
    #Go To The Panel-Body
        element = tree.xpath('//div[@class="space-8 space-top-8"]/descendant::text()[2]')[0]
        return str(element)
...
if __name__ == '__main__':
    ...
    MainResults = IterateMainPage('Dallas-TX?guests=4&room_types%5B%5D=Entire+home%2Fapt', 1)
